...
async def safe_edit_or_send(query, text, reply_markup=None, parse_mode=None):
    try:
...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("sub_req_"):
        ...
        async def reply_fn(text, parse_mode=None):
            await safe_edit_or_send(q, text, parse_mode=parse_mode, reply_markup=back_user_kb(uid))
    ...
    if data == "my_account":
        u = get_user_cached(uid)
        if not u:
            await safe_edit_or_send(q, "❌ Account not found. Send /start first.", reply_markup=back_user_kb(uid))
            return
        lang  = get_lang(uid)
        is_ar = lang == "ar"
        ...
        await safe_edit_or_send(q, text, parse_mode="HTML", reply_markup=back_user_kb(uid))
        return
    ...
    if data in ("ni_name", "ni_national_id"):
        if not can_search(uid):
            await safe_edit_or_send(q, "❌ No searches remaining.", reply_markup=back_user_kb(uid))
            return
    ...
    if data == "show_plans":
        ...
        await safe_edit_or_send(q, text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup([
    ...
    if data == "show_help":
        ...
        await safe_edit_or_send(q, text, parse_mode="Markdown", reply_markup=back_user_kb(uid))
        return
    ...
    if data.startswith("confirm_nameid:"):
        ...
        _, stype, keyword = parts_cb
        context.user_data["search_type"] = stype
        await safe_edit_or_send(q, "⏳ جاري تحضير الملف...", parse_mode="Markdown")
        await do_nameid_search(update, context, keyword, stype, reply_to=q.message)
        return

    if data.startswith("cancel_search"):
        await safe_edit_or_send(q, "❌ Search cancelled.", reply_markup=new_search_kb())
        return
    ...
    if data == "adm_stats":
        try:
            ...
            await safe_edit_or_send(q, "❌ Stats error.", reply_markup=back_admin_kb())
            return
    ...
    if data == "adm_users" or data.startswith("adm_users_p") or data.startswith("adm_users_f"):
        ...
        try:
            ...
            await safe_edit_or_send(q, "❌ Error loading users.", reply_markup=back_admin_kb())
            return
        ...
        await safe_edit_or_send(q, text[:4000], parse_mode="HTML", reply_markup=InlineKeyboardMarkup(kb_rows))
        return
    ...
    if data == "adm_filelist" or data.startswith("adm_filelist_p"):
        ...
        kb_rows = [nav_buttons] if nav_buttons else []
        kb_rows.append([InlineKeyboardButton("🔙 Admin Panel", callback_data="adm_home")])
        await safe_edit_or_send(q, text[:4000], parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(kb_rows))
        return
    ...
    if data == "adm_reset_daily":
        do_daily_reset()
        await safe_edit_or_send(q, "✅ Daily limits reset for all users (Search DB + Name/ID).", reply_markup=back_admin_kb())
        return
    ...
    if data == "my_history":
        ...
        await safe_edit_or_send(q, txt[:4000], parse_mode="Markdown", reply_markup=kb)
        return
    ...
    if data == "clear_history":
        ...
        await safe_edit_or_send(q, txt, parse_mode="Markdown", reply_markup=back_user_kb(uid))
        return
    ...
    if data == "adm_export_csv":
        ...
        await safe_edit_or_send(q, "✅ CSV sent to you.", reply_markup=back_admin_kb())
        return
    ...
    if data == "adm_backup":
        try:
            ...
            await safe_edit_or_send(q, f"❌ Backup failed: `{e}`", parse_mode="Markdown", reply_markup=back_admin_kb())
        return
    ...
    if data == "adm_sub_requests":
        ...
        await safe_edit_or_send(q, text[:4000], parse_mode="HTML", reply_markup=kb)
        return
    ...
    if data == "adm_op_logs":
        ...
        await safe_edit_or_send(q, text[:4000], parse_mode="HTML", reply_markup=back_admin_kb())
        return
    ...
    if data == "adm_toggle_maintenance":
        ...
        await safe_edit_or_send(q, status, parse_mode="Markdown", reply_markup=back_admin_kb())
        return
    ...
    if data == "adm_delete_file":
        ...
        if not files:
            await safe_edit_or_send(q, "No files to delete.", reply_markup=back_admin_kb())
            return
        text = "🗑️ *Delete File*\n\nSend the file ID:\n\n"
